...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer: Yes, it appears that they have the same column names though some are in different orders. In some, the "Total" column appears in all caps, and in 2022 it does not, so we will have to clean that.**
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: There are now 24 rows. I think this is correct because that matches our previous data table.**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer: Now we have information on populations of ages and races for each county.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer: Prince George's County: Percent black and 18+: 60.9, Percent democrats: 77.095.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer: Number**
