...
**Task** Run the following code and describe the dataframe it outputs. **Answer: This data has dates, titles, and urls of Ben Cardin's press releases.**
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer: It picks out each unique word, but classifies the same word with different capitalization as different words.**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer: Each row is its own word from the press releases, all are lowercase.**
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer: The graph shows which words are used the most often in Cardin's press releases. It does include the words we don't care about, such as "the" and "of".**
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer: Yes, this data is more interesting than before because we now only see the words that really matter instead of words like "the".**
...
**Task** Run the following code and describe the results. **Answer: We now have a separate data set for each year's top ten most used unique words in Ben Cardin's press releases. We also have the number of times the word was used each year and the percentage of the total words that this word accounts for.**
...
**Task** Run the following code and describe the results. **Answer: We now see the top ten most used two-word phrases in Ben Cardin's press releases.**
...
**Task** Run the following code and describe the results. **Answer: We now see the top ten most used two-word phrases in Ben Cardin's press releases in 2022.**
...
**Task** Run the following code and describe the results. **Answer: We now see the top ten most used two-word phrases in Ben Cardin's press releases in 2023.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer: We now have datasets for each year that tell us the sentiments of each word, such as positive or negative.**
