...
**A2. The pattern of my results shows a spike in week 28, which is in the summertime.**
...
pg_calls_by_week |>
  ...
  labs(
    title = 'Prince George\'s County overdose calls spike in the summer during the 28th week of the year.',
    x = 'Week',
    y = 'Percentage of calls',
    caption = 'Source: PG County EMS',
  )
...
**A3. This scatterplot is showing that areas with lower median incomes in PG county see more 911 calls, whereas areas with higher median incomes generally see f ewer 911 calls. I would report these points by investigating further the nature of these calls and possible causes.**

```{r}
pg_calls_by_zip <- pg_calls |>
  group_by(zipcode)|>
  summarize(total_calls = n())|>
  arrange(desc(total_calls))
...
pg_calls_with_income <- pg_calls_by_zip |> inner_join(md_median_income_zip, by=c('zipcode'='GEOID'))
...
pg_calls_with_income |>
  ...
  labs(
    title="Areas with lower median incomes in Prince George's County have more 911 Overdose Calls",
    y = "median income",
