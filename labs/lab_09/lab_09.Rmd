...
**A1.** <https://www.datawrapper.de/_/jJEw1/>

I created a double bar chart to group by county and easily compare the
different percentages for Cox and Hogan for each county.
...
all_md_gov_county <- md_gov_county |> left_join(md_gov_county_18, join_by("geoid", "county")) |>
mutate(
    total_votes.x = cox + moore + lashar + wallace + harding + sefcik + write_ins.x,
    pct_cox = (cox/total_votes.x)*100
  ) |>
mutate(
    total_votes.y = hogan + jealous + quinn + schlakman + write_ins.y,
    pct_hogan = (hogan/total_votes.y)*100
  )

write.csv(all_md_gov_county, "all_md_gov_county.csv")
```
...
<https://www.datawrapper.de/_/guhIs/>

The story here is that the percentage of mail-in ballots rose during the
pandemic in 2020 and fell again in 2022, where the percentage of
election day voting rose.
...
<https://www.datawrapper.de/_/HwNdd/>

I think the story here is that more urban areas of Montgomery County
have more overdose calls.
