...
```{r}
emergency_calls <- read_csv("lab_05/tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |> 
  clean_names() |>
...
A1. July 14 and October 4 had the most overdose calls in 2022 with 23 calls for each day. I don't think there are any dates that had zero calls, as I see 366 rows. We can't know for sure because the data starts in February 2022, so we don't know what happened in January.
...
A2. The days with the highest percentages of calls are Friday, Saturday, and Sunday, which makes sense as many people are probably using drugs more during the weekend.
...
A3. 4540 Silver Spring Rd has the most overdose calls with 36 calls. After Googling the address, I found that the house was recently sold, so I assume the person living there during these calls no longer lives there. The next two most called from places are police departments, which makes sense. I might be less confident about the counts if the data was not entered cleanly, such as someone mistyping an address or typing it with a variation.

```{r}
emergency_calls |>  
  group_by(location) |>
  summarize(location_count = n()) |>
  mutate(pct_location = (location_count/sum(location_count)*100)) |>
...
A4. I would definitely want to know more about the house that has seen 36 calls and would like to know if that person is okay or doing better. It might also be cool to do a story on the increased calls over the weekends.
