...
combined_911 <- combine(allegany_911, cecil_911, carroll_911)
```
...
A2. An outlier is 314 Grove Neck Rd, which had the most calls with 15. This makes sense because after Googling the address, I learned that it is a rehab center.
...
A3. The zip code with the most calls is 21217. Some neighborhoods in this zip code are Penn North, Harlem Park and Druid Heights.
...
maryland_zcta |>
  summarize(avg_pct_rent = mean(pct_rent))
```
...
A4: Find the median ages for the zip codes that have the highest and lowest percentage of calls out of their populations.

The zip code with the highest percentage of calls, 21217, has a median age of 34.3. The zip code with the lowest percentage of calls, 21228, has a median age of 43.4.

```{r}
baltimore_911_with_demographics |>
 group_by(zip_code,population,medage) |>
  summarize(calls = n()) |>
  mutate(percentage = (calls/population)*100) |>
